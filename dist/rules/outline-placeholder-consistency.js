...
export default class OutlinePlaceholderConsistency {
    name = 'outline-placeholder-consistency';
    ...
    async run(document) {
        document.feature.children.forEach((child) => {
            ...
            if (examples.length === 0)
                return;
            const stepsPlaceholders = new Set();
            ...
            examples.forEach((example) => {
                ...
                for (const name of headerNames) {
                    if (!stepsPlaceholders.has(name)) {
                        document.addError(this, `Placeholder "<${name}>" appears in Examples but not in any step.`, header.location);
                    }
                }
            });
            for (const name of stepsPlaceholders) {
                if (!allHeaderNames.has(name)) {
                    document.addError(this, `Placeholder "<${name}>" is used in steps but not in any Examples header.`, child.scenario.location);
                }
            }
        });
    }
}
//# sourceMappingURL=outline-placeholder-consistency.js.map