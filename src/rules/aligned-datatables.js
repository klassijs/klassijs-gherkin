...
var AlignedDatatables = /** @class */ (function () {
    function AlignedDatatables(rawSchema) {
        this.name = 'aligned-datatables';
        this.acceptedSchema = schemas_js_1.switchOrSeveritySchema;
        this.schema = new schema_js_1.default(rawSchema);
    }
    AlignedDatatables.prototype.run = function (document) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            return __generator(this, function (_a) {
                document.lines.forEach(function (line, lineIndex) {
                    // Only consider lines that start with a '|' after trimming left padding
                    if (line.text.trimStart().charAt(0) !== '|') {
                        return;
                    }
                    var nextLine = document.lines[lineIndex + 1];
                    if (!nextLine)
                        return;
                    // Collect pipe positions for the current line
                    var pipeIndices = [];
                    var pipeIndex = line.text.indexOf('|');
                    while (pipeIndex !== -1) {
                    ...
                    var nextPipeIndices = [];
                    var nextPipeIndex = nextLine.text.indexOf('|');
                    ...
                    pipeIndices.forEach(function (col, idx) {
                        var matchCol = nextPipeIndices[idx];
                        if (matchCol === undefined)
                            return;
                        if (col !== matchCol) {
                            document.addError(_this, 'Data table is not formatted correctly', {
    ...
    return AlignedDatatables;
}());
exports.default = AlignedDatatables;
