...
import type { GherklinConfiguration } from '../types.js'
...
async function main() {
    ...
    const { config, filepath } = await resolveConfig<GherklinConfiguration>({
    ...
    if (!config) {
        console.error(
            'No Gherklin config found. Add a gherklin.config.yaml (or .ts/.yml) in the project root or pass --config <path>.',
        )
        process.exit(2)
    }
